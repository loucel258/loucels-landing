...
const BASE_URL = process.env.NEXT_PUBLIC_SITE_URL ?? "https://loucels.com";
const CAL_URL = "https://cal.com/loucels/discovery";
...
async function sendStep1(sub: SubscriberRow): Promise<boolean> {
  ...
  const subject = isEs
    ? "Tu Trust Stack one-pager — Loucels"
    : "Your Trust Stack one-pager — Loucels";
...
function shellHtml(body: string): string {
  return `<!doctype html><html><head><meta charset="utf-8"></head><body style="font-family:-apple-system,system-ui,sans-serif;font-size:15px;line-height:1.55;color:#1f2937;max-width:560px;margin:24px auto;padding:0 16px;">${body}<hr style="margin:32px 0;border:none;border-top:1px solid #e5e7eb;"><p style="font-size:12px;color:#9ca3af;">Loucels · Palm Beach County, FL · <a href="${BASE_URL}" style="color:#06b6d4;">loucels.com</a></p></body></html>`;
}
...
function renderStep1En(): string {
  return shellHtml(`
    ...
    <p>It walks through the 5 layers we build into every Loucels agent — DLP, audit chain, RBAC, HITL, hardened prompts.</p>
...
function renderStep1Es(): string {
  return shellHtml(`
    ...
    <p>Recorre las 5 capas que construimos en cada agente Loucels — DLP, audit chain, RBAC, HITL, prompts blindados.</p>
